...
version = re.search(
    "^__version__\s*=\s*'(.*)'",
    open('SearchAnalyzer/version.py').read(),
    re.M).group(1)

requirements = [r for r in open('requirements.txt', 'r').read().split('\n') if r]

setup(name='SearchAnalyzer',
      version=version,
      ...
      py_modules=['usage'],
      packages=['SearchAnalyzer'],
      entry_points={'console_scripts': ['SearchAnalyzer = SearchAnalyzer.core:main']},
